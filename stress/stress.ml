...
module Test(Store : S.STORE) = struct
  ...
  let create_builder ~proc_mgr store conf =
    let module Builder = Obuilder.Builder(Store)(Native_sandbox)(Obuilder.Docker_extract) in
    let sandbox = Native_sandbox.create ~proc_mgr ~state_dir:(Store.state_dir store / "sandbox") conf in
  ...
  let stress_builds ~sw ~proc_mgr store conf =
    let builder = create_builder ~proc_mgr store conf in
  ...
  let prune ~proc_mgr store conf =
    let (Builder ((module Builder), builder)) = create_builder ~proc_mgr store conf in
...
let stress (sandbox, spec) conf =
  ...
  T.stress_builds ~sw ~proc_mgr store conf;
  T.prune ~proc_mgr store conf
