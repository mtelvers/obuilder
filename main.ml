...
let create_builder ~proc_mgr store_spec conf =
...
let select_backend ~proc_mgr (sandbox, store_spec) native_conf docker_conf qemu_conf =
  match sandbox with
  | `Native -> create_builder ~proc_mgr store_spec native_conf
...
let build () store spec native_conf docker_conf qemu_conf src_dir secrets =
  ...
  let (Builder ((module Builder), builder)) = select_backend ~proc_mgr store native_conf docker_conf qemu_conf in
