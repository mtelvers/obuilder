...
module type SANDBOX = sig
  ...
  (** [run ~sw t config dir] runs the operation [config] in a sandbox with root
      filesystem [dir].
      @param sw Switch for cancellation - cancelling the switch kills the process (and returns [`Cancelled]).
