...
let get_cache t name =
  ...
  | Some c -> c
  | None ->
      let c = { lock = Lwt_mutex.create ()} in
...
let cache ~user:_ t name =
  ...
  | `Missing -> Overlayfs.create ~mode:"1777" [ master ]
  | `Present -> Lwt.return_unit)
  >>= fun () ->
  Overlayfs.create ~mode:"1777" [ result; work; merged ] >>= fun () ->
...
let delete_cache t name =
  ...
  if Sys.file_exists snapshot then
    Overlayfs.delete [ snapshot ] >>= fun () -> Lwt_result.return ()
