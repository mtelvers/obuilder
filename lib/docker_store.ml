...
module Cache : sig
  ...
  let exists volume =
    Result.is_ok (Docker.Cmd.exists volume)

  let create volume =
    let id = Docker.Cmd.volume (`Create volume) in
  ...
  let delete volume =
    ignore (Docker.Cmd.volume (`Remove [volume]))
end
...
let purge () =
  ...
  if volumes <> [] then ignore (Docker.Cmd.volume (`Remove volumes))
...
let result t id =
  ...
  match Docker.Cmd.exists img with
...
let cache ~user t name =
  ...
  if not (Cache.exists snapshot) then Cache.create snapshot;
  (* Create writeable clone. *)
  let gen = cache.gen in
  Cache.snapshot ~src:snapshot tmp;
  (match user with
   | `Unix { Obuilder_spec.uid; gid } ->
     let tmp_path = Docker.Cmd.mount_point tmp in
     Os.sudo ["chown"; strf "%d:%d" uid gid; tmp_path]
   | `Windows _ -> () (* FIXME: does Windows need special treatment? *));
  let release () =
    Eio.Mutex.use_rw ~protect:false cache.lock @@ fun () ->
    if cache.gen = gen then begin
