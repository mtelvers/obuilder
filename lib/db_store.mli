module Make (Raw : S.STORE) : sig
  ...
  val build :
    ...
    id:S.id ->
    log:S.logger ->
    (cancelled:unit Eio.Promise.t -> log:Build_log.t -> string -> (unit, [`Cancelled | `Msg of string]) result) ->
  ...
  (** [build t ~id ~log fn] ensures that [id] is cached, using [fn ~cancelled ~log dir] to build it if not.
      ...
      @param sw Release this switch if you no longer need the result. The build
