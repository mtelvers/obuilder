...
let create_new_user ~username ~home_dir ~uid ~gid =
  ...
  if exists then Ok ()
    ...
    match sudo_result ~pp:(pp "UniqueID") (dscl @ [ "UniqueID"; uid ]) with
    | Error _ as e -> e
    | Ok _ ->
      match sudo_result ~pp:(pp "PrimaryGroupID") (dscl @ [ "PrimaryGroupID"; gid ]) with
      | Error _ as e -> e
      | Ok _ ->
        match sudo_result ~pp:(pp "UserShell") (dscl @ [ "UserShell"; "/bin/bash" ]) with
        | Error _ as e -> e
        | Ok _ ->
          match sudo_result ~pp:(pp "NFSHomeDirectory") (dscl @ [ "NFSHomeDirectory"; home_dir ]) with
          | Error _ as e -> e
          | Ok _ -> Ok ()
...
let delete_user ~user =
  ...
  match exists with
    ...
    | true ->
      ...
      match sudo_result ~pp:(pp "Deleting") delete with
      | Error _ as e -> e
      | Ok _ -> Ok ()
...
let rec kill_users_processes ~uid =
  ...
  match sudo_result ~pp:(pp "PKILL") delete with
...
let rec sudo_fallback cmds cmds2 ~uid =
  ...
  match sudo_result ~pp cmds with
  | Ok _ -> ()
  | Error (`Msg m) ->
...
let rm ~directory =
  ...
  match sudo_result ~pp:(pp "RM") delete with
  | Ok _ -> ()
  | Error (`Msg m) ->
