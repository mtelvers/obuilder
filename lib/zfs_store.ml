...
module Dataset : sig
  ...
  let if_missing ?snapshot t ds fn =
    if not (exists ?snapshot t ds) then fn ()
end
...
module Zfs = struct
  ...
  let mount ?snapshot t ~ds =
    if not (mounted t ~ds ?snapshot) then begin
      let pp _ ppf = Fmt.pf ppf "zfs mount" in
      match Os.sudo_result ~pp:(pp "zfs mount") ~is_success:(fun n -> n = 0 || n = 16) ["zfs"; "mount"; "--"; Dataset.full_name t ds ?snapshot] with
      | Ok () -> ()
      | Error (`Msg m) ->
...
let result t id =
  let ds = Dataset.result id in
  if Dataset.exists t ds ~snapshot:default_snapshot then begin
    Zfs.mount t ~ds;
...
let delete_cache t name =
  ...
  if cache.n_clones > 0 then Error `Busy
    ...
    let main_ds = Dataset.cache name in
    if Dataset.exists t main_ds then begin
