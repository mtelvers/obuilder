...
module Context = struct
  ...
  let v ?sw ?(env=[]) ?(user=Obuilder_spec.root) ?workdir ?shell ?(secrets=[]) ~log ~src_dir () =
    ...
    { sw; env; src_dir; user; workdir; shell; log; scope = Scope.empty; secrets }
...
module Make (Raw_store : S.STORE) (Sandbox : S.SANDBOX) (Fetch : S.FETCHER) = struct
  ...
  let run t ~sw ~log ~cache run_input =
    let id =
    ...
    Store.build t.store ?sw ~base ~id ~log (fun ~cancelled ~log result_tmp ->
        Eio.Switch.run @@ fun sandbox_sw ->
        (* Set up cancellation: when the cancelled promise resolves, cancel the sandbox switch *)
        Eio.Fiber.fork ~sw:sandbox_sw (fun () ->
          ignore (Eio.Promise.await cancelled);
          Eio.Switch.fail sandbox_sw (Failure "Build cancelled"));
        let to_release = ref [] in
        Fun.protect
          (fun () ->
             let mounts = List.map (fun { Obuilder_spec.Cache.id; target; buildkit_options = _ } ->
             ...
             Unix.close close_me;
             Sandbox.run ~sw:sandbox_sw ~stdin ~log t.sandbox config result_tmp
          )
          ~finally:(fun () ->
             List.iter (fun f -> f ()) !to_release
          )
      )
  ...
  let copy t ~context ~base { Obuilder_spec.from; src; dst; exclude } =
    let { Context.sw; src_dir; workdir; user; log; shell = _; env = _; scope; secrets = _ } = context in
    ...
    let src_dir_result =
      ...
      | `Build name ->
        ...
        | Some id ->
          ...
          | None -> Error (`Msg (Fmt.str "Build result %S not found" id))
          | Some dir -> Ok (dir / "rootfs")
    in
    ...
    | Ok src_dir ->
      ...
      | Ok op ->
        ...
        Store.build t.store ?sw ~base ~id ~log (fun ~cancelled ~log result_tmp ->
            ...
            Eio.Fiber.both
              (fun () ->
                 result := Sandbox.run ~sw:sandbox_sw ~stdin:from_us ~log t.sandbox config result_tmp)
              (fun () ->
  ...
  let rec run_steps t ~(context:Context.t) ~base = function
    ...
    | op :: ops ->
      ...
      | `Run { shell = cmd; cache; network; secrets = mount_secrets } ->
        let result =
          let { Context.sw; workdir; user; env; shell; log; src_dir = _; scope = _; secrets } = context in
          resolve_secrets secrets mount_secrets |> Result.map @@ fun mount_secrets ->
          (sw, { base; workdir; user; env; cmd; shell; network; mount_secrets }, log)
        in
        begin match result with
        | Error _ as e -> e
        | Ok (sw, run_input, log) ->
          match run t ~sw ~log ~cache run_input with
          | Error _ as e -> e
          | Ok base -> k ~base ~context
        end
      | `Copy x ->
        begin match copy t ~context ~base x with
        | Error _ as e -> e
        | Ok base -> k ~base ~context
        end
      | `Env ((key, _) as e) ->
  ...
  let healthcheck ?(timeout=30.0) t =
    ...
    | Ok () ->
      ...
      | Ok (id, env) ->
        ...
        let context = Context.v ~sw ~log ~src_dir:"/tmp" () in
        let context = { context with env } in
        (* Start the timer in a fiber *)
        let timed_out = ref false in
        Eio.Fiber.fork ~sw (fun () ->
            Unix.sleepf timeout;
            timed_out := true
          );
        if !timed_out then Fmt.error_msg "Timeout running healthcheck"
        else
...
module Make_Docker (Raw_store : S.STORE) = struct
  ...
  let run t ~sw ~log ~cache run_input =
    ...
    in
    let { base; workdir; user; env; cmd; shell; network; mount_secrets } = run_input in
    Store.build t.store ?sw ~base ~id ~log (fun ~cancelled ~log _ ->
        Eio.Switch.run @@ fun sandbox_sw ->
        (* Set up cancellation *)
        Eio.Fiber.fork ~sw:sandbox_sw (fun () ->
          ignore (Eio.Promise.await cancelled);
        ...
        Fun.protect
          (fun () ->
             ...
             Os.with_pipe_to_child @@ fun ~r:stdin ~w:close_me ->
             Unix.close close_me;
             match Docker_sandbox.run ~sw:sandbox_sw ~stdin ~log t.sandbox config id with
             | Ok () ->
    ...
    | Ok src_dir ->
      ...
      | Ok op ->
        ...
        Store.build t.store ?sw ~base ~id ~log (fun ~cancelled ~log _ ->
            ...
            match src_dir with
            | `Context src_dir ->
              Docker_sandbox.copy_from_context t.sandbox ~sw:sandbox_sw ~log op ~user ~src_dir ?dst_dir id
            | `Build (from_id, _) ->
              Docker_sandbox.copy_from_build t.sandbox ~sw:sandbox_sw ~log op ~user ~workdir ?dst_dir ~from_id id
          )
  ...
  let healthcheck ?(timeout=if Sys.win32 then 300.0 else 30.0) t =
    ...
    | Ok () ->
      ...
      | Ok (id, env) ->
        ...
        let context = Context.v ~sw ~log ~src_dir () in
        let context = { context with env } in
        (* Start the timer in a fiber *)
        let timed_out = ref false in
        Eio.Fiber.fork ~sw (fun () ->
