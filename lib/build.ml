    ...
    | Ok op ->
      ...
      Store.build t.store ?switch ~base ~id ~log (fun ~cancelled ~log result_tmp ->
          ...
          if Sys.win32 then begin
            (* On Windows, Lwt_unix.write to pipes hangs. Write tar data to a
               temp file first, then pass the file as stdin to the sandbox. *)
            let tar_file = result_tmp / "obuilder-tar-stdin.dat" in
            let tar_fd = Lwt_unix.of_unix_file_descr
                (Unix.openfile tar_file [Unix.O_WRONLY; Unix.O_CREAT; Unix.O_TRUNC] 0o600) in
            Lwt.finalize
              (fun () ->
                 match op with
                 | `Copy_items (src_manifest, dst_dir) ->
                   Tar_transfer.send_files ~src_dir ~src_manifest ~dst_dir ~to_untar:tar_fd ~user
                 | `Copy_item (src_manifest, dst) ->
                   Tar_transfer.send_file ~src_dir ~src_manifest ~dst ~to_untar:tar_fd ~user)
              (fun () -> Lwt_unix.close tar_fd)
            >>= fun () ->
            let stdin_fd = Unix.openfile tar_file [Unix.O_RDONLY] 0 in
            let stdin = { Os.raw = stdin_fd; needs_close = true } in
            Lwt.finalize
              (fun () -> Sandbox.run ~cancelled ~stdin ~log t.sandbox config result_tmp)
              (fun () ->
                 Os.ensure_closed_unix stdin;
                 (try Unix.unlink tar_file with Unix.Unix_error _ -> ());
                 Lwt.return_unit)
          end else begin
