...
let write_layerinfo ~dir ~snapshot_key ~source ~parent_layer_paths =
  ...
  let path = dir / "layerinfo.json" in
  Os.write_file ~path (Yojson.Safe.pretty_to_string json ^ "\n")
...
let fetch ~log:(_log : Build_log.t) ~root:(_root : string) ~rootfs base =
  ...
  (ctr_pread ["snapshot"; "prepare"; "--mounts"; key; chain_id] >>= function
   | Ok mounts_json ->
     ...
     write_layerinfo ~dir:rootfs ~snapshot_key:key ~source ~parent_layer_paths >>= fun () ->
